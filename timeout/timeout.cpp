...
void jFramework::timeout::setPeriod(int64_t time_in, timeUnit unit)
{
    switch (unit)
    {
    case MINUTES:
...
int64_t jFramework::timeout::getTimeout_s()
{
    return _time_in/(1000 * 1000);
}

int64_t jFramework::timeout::getTimeout_ms()
{
    return _time_in/(1000);
}

int64_t jFramework::timeout::getTimeout_us()
{
    return _time_in;
}
